...
namespace g3
{
    ...
    /// </summary>
    public class CutterCenterPositionSurface
    {
        public DMesh3 Mesh;
        public double CutterDiameter {get; set;}
        public double CutterCornerRadius {get; set;}
        ...
        public Func<Vector3d, Vector3d, int, Vector3d> CutterCenterPositionF;
        ...
        public CutterCenterPositionSurface(DMesh3 mesh)
        {
            Mesh = mesh;

            CutterCenterPositionF = (pos, normal, idx) => {
                Vector3d n = normal.Normalized;
                double xComponent = n.x * (CutterCornerRadius + Allowance + (CutterDiameter * 0.5 - CutterCornerRadius)/Math.Sqrt(n.x*n.x + n.y * n.y)) ;
                double yComponent = n.y * (CutterCornerRadius + Allowance + (CutterDiameter * 0.5 - CutterCornerRadius)/Math.Sqrt(n.x*n.x + n.y * n.y)) ;
                double zComponent = CutterCornerRadius * (n.z -1) + n.z * Allowance;
                Vector3d offsetVector = new Vector3d(xComponent,yComponent,zComponent);
                return pos + offsetVector;
            };
        }
        ...
        public virtual bool Extrude()
        {
            ...
            foreach (int vid in InitialVertices) {
                ...
                Vector3d n = (bHaveNormals) ? Mesh.GetVertexNormal(vid) : (Vector3d)normals.Normals[vid];
                Vector3d newv = CutterCenterPositionF(v, n, vid);
